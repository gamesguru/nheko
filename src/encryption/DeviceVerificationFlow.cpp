                                           ...
                                           std::string)
{
    ...
    if (msg.key_agreement_protocols && std::ranges::count(msg.key_agreement_protocols.value(), "curve25519-hkdf-sha256") &&
        msg.hashes && std::ranges::count(msg.hashes.value(), "sha256")) {
        if (msg.message_authentication_codes && std::ranges::count(msg.message_authentication_codes.value(), mac_method_alg_v2)) {
            this->mac_method = mac_method_alg_v2;
        } else if (msg.message_authentication_codes && std::ranges::count(msg.message_authentication_codes.value(), mac_method_alg_v1)) {
        ...
        if (msg.short_authentication_string && std::ranges::count(msg.short_authentication_string.value(),
                               mtx::events::msg::SASMethods::Emoji)) {
            this->method = mtx::events::msg::SASMethods::Emoji;
        } else if (msg.short_authentication_string && std::ranges::count(msg.short_authentication_string.value(),
